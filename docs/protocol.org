...
- *Topology:* Single server (Raspberry Pi), 1-247 sensors (ESP32).
- *Flow:* Server sends a POLL; addressed sensor replies with a REPLY.
  All other sensors stay silent.

** Push mode (UDP)

- *Transport:* UDP over WiFi.
- *Topology:* Single server listening on a UDP port, sensors push.
- *Flow:* Sensors wake periodically, send a REPLY frame via UDP to
  the server, then return to deep sleep.  No POLL needed.
- *Addressing:* The ADDR byte in the REPLY identifies the sensor.
...
| ADDR    | 1 byte  | Sensor address, 1-247                                 |
...
|----------+--------+-----------------+---------------------|
| POLL     | ~0x01~ | Server -> Sensor | None (LEN = 0)     |
| REPLY    | ~0x02~ | Sensor -> Server | 8 bytes (see below) |

** POLL (0x01)

Server requests a reading from the addressed sensor.  No payload.

** REPLY (0x02)

Sensor responds with its current temperature readings.  Fixed 8-byte
payload:
...
- No response within the timeout means the sensor is offline.  The
  server logs the event and moves to the next sensor.

* Error handling

- *CRC mismatch:* frame is silently discarded.
- *Unknown command:* sensor ignores the frame (no reply).
...
** Example 1: server polls sensor 3
...
** Example 2: sensor 3 replies, channels 0 and 1 valid
