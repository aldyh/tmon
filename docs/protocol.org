...
- *Flow:* Master sends a POLL; addressed slave replies with a REPLY.
  All other slaves stay silent.

*** Push mode (UDP)

- *Transport:* UDP over WiFi.
- *Topology:* Single master listening on a UDP port, slaves push.
- *Flow:* Slaves wake periodically, send a REPLY frame via UDP to
  the master, then return to deep sleep.  No POLL needed.
- *Addressing:* The ADDR byte in the REPLY identifies the slave.
