...
|------------+----------------------+-------------------|
| RS-485     | Production           | USB-RS485 adapter |
...
The ESP32-S3-DevKitC-1 has two USB-C connectors.  They serve different
purposes and appear as different device types on the host:
...
| Port  | Label | Device         | Used for                              |
|-------+-------+----------------+---------------------------------------|
| Left  | USB   | ~/dev/ttyACM*~ | Flashing firmware (~tmon-flash~)      |
| Right | COM   | ~/dev/ttyUSB*~ | UART bridge (not used in this project) |

*Use the left USB port for flashing.*  The ~tmon-flash~ tool
auto-detects ~/dev/ttyACM*~ devices for this reason.

*Avoid confusion with the USB→485 adapter:* The RS-485 adapter plugged
into the Raspberry Pi also appears as ~/dev/ttyUSB*~.  If the ESP32's
right UART port is connected at the same time, there will be two
~/dev/ttyUSB~ devices and it becomes unclear which is which.  Stick to
the left port on the ESP32 and this ambiguity goes away.

** NTC thermistor inputs

Channels 1 through 4 on GPIO 1 through 4:
...
** Testing without the MAX485 module (optional)
