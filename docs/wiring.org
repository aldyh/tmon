...
|---------------+--------------------------------------------|
| Off           | Normal idle                                |
| Red solid     | Error (watchdog timeout or no WiFi)        |
| Green blink   | Transmitting (brief flash, ~100ms)         |

**** RS-485 build (always-on)

The LED is normally off.  A brief green flash occurs when responding
to a POLL.  If no POLL is received for 90 seconds, the LED turns
solid red to indicate a watchdog timeout.

**** UDP push build (deep sleep)

The LED is normally off (deep sleep).  On wake:
- If WiFi fails to connect: solid red, then sleep
- On successful send: brief green flash, then sleep
...
Some USB-RS485 adapters have a built-in 120-ohm termination resistor.
