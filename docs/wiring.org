...
The ESP32-S3-DevKitC-1 has a built-in RGB LED.  Only the UDP firmware
uses it.  The serial firmware does not use the LED at all.

| Color         | Meaning                  |
|---------------+--------------------------|
| Off           | Normal (WiFi connected)  |
...
ESP32 stays connected to WiFi and pushes readings to the master over
