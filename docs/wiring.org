...
| WiFi       | Wireless alternative    | TCP over network         |
...
to signal connection state.  No external wiring required.

**** UART and WiFi TCP builds (always-on)

| Color/Pattern  | Meaning                                    |
|----------------+--------------------------------------------|
| Green solid    | Communication active (recent POLL)         |
| Yellow solid   | Waiting for first POLL                     |
| Red slow blink | Watchdog timeout, no POLL for 90s (1 Hz)   |
...
| Green flash   | Sent reading (brief, ~200ms)   |
| Off           | Deep sleep between pushes      |

The UDP build spends most of its time in deep sleep, so the LED is
usually off.  A brief green flash indicates a successful send.
...
ESP32 connects to the master over TCP, using the same binary protocol.
