...
| Status LED     | GPIO 48    | Built-in WS2812 RGB LED          |
...
The ESP32 firmware still toggles GPIO 5 for DE/RE timing, but since
...
The ESP32-S3-DevKitC-1 has a built-in WS2812 RGB LED on GPIO 48.  The
firmware uses it to signal connection state:
...
** WiFi transport (alternative to RS-485)
...
WiFi builds use ~main_wifi.cpp~ instead of ~main.cpp~.  The UART and
MAX485 pins are unused; only the NTC inputs and status LED apply.

Build with ~make build-slave-wifi~ and configure SSID/password via
build flags in ~platformio.ini~ or environment variables.

See ~master/config-wifi.toml~ for master-side WiFi configuration.
