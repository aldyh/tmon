...
|-------+----------------+---------------------------------------|
| Right | ~/dev/ttyACM*~ | Flashing firmware (~tmon-flash~)      |
| Left  | ~/dev/ttyUSB*~ | UART bridge (not used in this project) |

*Use the right USB port for flashing.*  The ~tmon-flash~ tool
...
left UART port is connected at the same time, there will be two
~/dev/ttyUSB~ devices and it becomes unclear which is which.  Stick to
the right port on the ESP32 and this ambiguity goes away.
