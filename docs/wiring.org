...
Build with ~make build-slave-wifi~.  WiFi credentials are read from
~master/wifi.toml~ (copy from ~wifi.toml.example~).

Master-side WiFi settings are in ~master/config-wifi.toml~.
