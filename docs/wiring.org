...
thermistors.  Two transport options connect sensors to the server:
...
| Function       | ESP32 GPIO |
|----------------+------------|
| NTC channel 0  | GPIO 1     |
| NTC channel 1  | GPIO 2     |
| NTC channel 2  | GPIO 3     |
| NTC channel 3  | GPIO 4     |
...
to the Pi's GPIO UART (GPIO 14 TX, GPIO 15 RX), crossing TX/RX as
usual.  Leave GPIO 5 (DE/RE) unconnected -- the firmware toggles it
but nothing reads it without a transceiver on the other end.
...
WiFi sensors need only the common wiring (thermistors + power) -- no
