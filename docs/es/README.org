...
[wifi]
port = 5555               # puerto de escucha para clientes WiFi
push_interval = 5         # frecuencia con que los clientes envían lecturas (segundos)
#+end_example

Para clientes WiFi, también se necesita ~/etc/tmon/wifi.toml~ con las credenciales de red:
...
Habilite los dos daemons (uno para WiFi y otro para serial):

#+begin_example
sudo systemctl enable --now tmond-serial   # servidor RS-485
sudo systemctl enable --now tmond-wifi     # servidor WiFi push
#+end_example
