...
El instalador pregunta si desea habilitar clientes RS-485 y/o WiFi,
luego solicita la configuración correspondiente (puerto, direcciones,
intervalos).  Escribe los archivos de configuración, instala todo e
inicia los servicios -- no se necesita edición manual.
...
#+begin_example
sudo tmon-flash --mode=485 --addr=1       # cliente RS-485 en la dirección 1
sudo tmon-flash --mode=wifi --addr=5      # cliente WiFi en la dirección 5
#+end_example

La herramienta detecta automáticamente el puerto, pero se puede usar ~--port=/dev/ttyACM0~ para especificarlo manualmente.

*Puerto USB:* El ESP32-S3 DevKitC-1 tiene dos puertos USB-C.  Conecte
al *izquierdo* (marcado USB en la PCB) -- aparece como ~/dev/ttyACM*~.
El puerto UART derecho (marcado COM) aparece como ~/dev/ttyUSB*~ y
puede entrar en conflicto con el adaptador USB→485.  Vea la
[[file:wiring.org][Referencia de cableado]] para un diagrama.
