...
|----------+----------------+-----------------------------------------|
| Derecho  | ~/dev/ttyACM*~ | Flashear firmware (~tmon-flash~)        |
| Izquierdo | ~/dev/ttyUSB*~ | Bridge UART (no se usa en este proyecto) |

*Use el puerto USB derecho para flashear.*  La herramienta ~tmon-flash~
...
puerto UART izquierdo del ESP32 está conectado al mismo tiempo, habrá
dos dispositivos ~/dev/ttyUSB~ y no será claro cuál es cuál.  Use el
puerto derecho del ESP32 y esta ambigüedad desaparece.
