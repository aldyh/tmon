...
Stores one row per successful REPLY frame received from a sensor.
...
| addr   | INTEGER | Sensor address from the REPLY frame (1-247).                                         |
...
|----------------------+------------+----------------------------------------------------|
| idx_readings_addr_ts | (addr, ts) | Speeds up queries that filter by sensor address and |
