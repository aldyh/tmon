...
def _make_reply(addr, status, t0, t1, t2, t3):
    ...
    """
    payload = struct.pack("<Bhhhh", status, t0, t1, t2, t3)
...
class TestPollSlave:
    ...
    def test_success(self):
        ...
        reply = _make_reply(3, 0x03, 235, 198, PROTO_TEMP_INVALID,
                            PROTO_TEMP_INVALID)
        bus = FakeBus([reply])
    ...
    def test_wrong_addr(self):
        ...
        reply = _make_reply(5, 0x01, 100, PROTO_TEMP_INVALID,
    ...
    def test_all_channels_valid(self):
        ...
        reply = _make_reply(1, 0x0F, 100, 200, 300, 400)
        bus = FakeBus([reply])
        storage = Storage(":memory:")
        poller = Poller(bus, storage, [1])
    ...
    def test_negative_temps(self):
        ...
        reply = _make_reply(1, 0x01, -100, PROTO_TEMP_INVALID,
                            PROTO_TEMP_INVALID, PROTO_TEMP_INVALID)
        bus = FakeBus([reply])
        storage = Storage(":memory:")
        poller = Poller(bus, storage, [1])
    ...
    def test_sends_poll_frame(self):
        ...
        reply = _make_reply(3, 0x01, 100, PROTO_TEMP_INVALID,
                            PROTO_TEMP_INVALID, PROTO_TEMP_INVALID)
        bus = FakeBus([reply])
        storage = Storage(":memory:")
        poller = Poller(bus, storage, [3])
...
class TestRunOnce:
    ...
    def test_polls_all_slaves(self):
        ...
        reply1 = _make_reply(1, 0x01, 100, PROTO_TEMP_INVALID,
                             PROTO_TEMP_INVALID, PROTO_TEMP_INVALID)
        reply2 = _make_reply(2, 0x01, 200, PROTO_TEMP_INVALID,
                             PROTO_TEMP_INVALID, PROTO_TEMP_INVALID)
