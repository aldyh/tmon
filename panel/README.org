...
A web dashboard for tmon (Flask + Chart.js).

** Quick start

#+begin_example
make panel-server           # generate mock data + start server
#+end_example
...
| ~GET /api/current~                         | Latest reading per client (JSON)             |
| ~GET /api/history?addr=N&hours=H&points=P~ | Historical readings, downsampled to P points |
| ~GET /api/sensors~                         | List of distinct client addresses             |
...
- ~generate_data.py~ creates ~tmon_mock.db~ with the same schema as
  ~server/src/tmon/storage.py~.  3 clients, 2-4 channels each,
