...
class TestApiCurrent:
    ...
    def test_returns_latest_per_client(self, client):
        """Each client appears once with its most recent reading."""
        resp = client.get("/api/current")
        assert resp.status_code == 200
        data = json.loads(resp.data)
        addrs = [r["addr"] for r in data]
        assert sorted(addrs) == [1, 2]
        # Latest for client 1 is the 12:01:00 row
...
class TestApiHistory:
    ...
    def test_returns_history(self, client):
        """Returns readings for the specified client within the time window."""
        resp = client.get("/api/history?addr=1&hours=1")
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert len(data) == 3  # 3 rows for client 1

    def test_empty_for_unknown_client(self, client):
        """Unknown client address returns empty list."""
...
class TestApiSensors:
    ...
    def test_returns_client_list(self, client):
        """Returns distinct client addresses."""
...
class TestApiExport:
    ...
    def test_requires_addr(self, client):
        """Missing addr returns 400."""
        resp = client.get("/api/export?hours=1")
        assert resp.status_code == 400

    def test_requires_hours(self, client):
        """Missing hours returns 400."""
        resp = client.get("/api/export?addr=1")
        assert resp.status_code == 400

    def test_invalid_hours(self, client):
        """Non-positive hours returns 400."""
        resp = client.get("/api/export?addr=1&hours=-1")
        assert resp.status_code == 400
    ...
    def test_unknown_node(self, client):
        """Unknown node returns header-only CSV."""
        resp = client.get("/api/export?addr=99&hours=24")
        assert resp.status_code == 200
        reader = csv.reader(io.StringIO(resp.data.decode()))
        rows = list(reader)
        assert len(rows) == 1
    ...
    def test_empty_db(self, empty_client):
        ...
        resp = empty_client.get("/api/export?addr=1&hours=24")
        assert resp.status_code == 200
        reader = csv.reader(io.StringIO(resp.data.decode()))
        rows = list(reader)
        assert len(rows) == 1
