...
- 1-4 ESP32 clients, each with up to 4 NTC thermistors.
...
- *Config:* ~tmon.toml~ (stdlib ~tomllib~ in Python 3.11), transport selected via ~--transport~ CLI flag.
- *Testing:*
  - Unit tests: ~pytest~ with fakes/mocks (no hardware).
  - Integration tests: ~socat~ + serial simulator (~server/tools~).
- *Firmware (client):* PlatformIO + Arduino framework; keep firmware straightforward.
...
- [X] Add stub tests so that we can "run" tests for client, server, etc, even if just placeholders.
...
- [X] Add root Makefile with ~check~, ~check-server~, ~check-client~ targets
- [X] Simplify README.org testing section to point at ~make check~
- [X] Add ~all~, ~build-server~, ~build-client~ targets to Makefile
- [X] Rewrite README Quick Start to use make targets

*** Simulator (x86)

- [X] Implement a simple "virtual client" that speaks the protocol over a pseudo-serial port
...
- [X] Client: NTC thermistor reading (Steinhart-Hart)
- [X] Client: UDP push with deep sleep
- [X] Client: Build environments (uart vs udp)
...
- [X] systemd service files: ~tmond-485.service~, ~tmond-udp.service~
...
- [X] ~tmon-flash~ script: ~--mode={485,wifi} --addr=N~
...
- [X] ~Makefile~: 2 generic builds, ~flash-client*~ calls ~tmon-flash~
...
*** Rename to server/client terminology
...
- [ ] Server polls client over serial, receives valid temperature readings
...
- [ ] Real bus smoke test with 1 client -> then scale to all clients

*** Integration + polish

- [X] Document how to add a new client (address + wiring + config)
...
- [X] Final pass: remove unused code, clean up comments, ensure docs match behavior
