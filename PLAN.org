...
- [X] Master: WifiBus TCP server implementation
- [X] Master: Config transport selection (rs485 or wifi)
- [X] Master: WiFi integration tests
- [X] Slave: Full protocol handler (respond to POLL with REPLY)
- [X] Slave: NTC thermistor reading (Steinhart-Hart)
- [X] Slave: WiFi client connecting to master
- [X] Slave: Build environments (uart vs wifi)
- [X] Makefile targets for WiFi builds
