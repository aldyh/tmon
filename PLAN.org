...
- [ ] Define layout: ~/etc/tmon/~ for config, ~/var/lib/tmon/~ for data + firmware
- [ ] Config path resolution: check repo-local first, fall back to ~/etc/tmon/~
- [ ] Database path: repo-local ~tmon.db~ for dev, ~/var/lib/tmon/tmon.db~ for production
- [ ] Panel reads DB from same resolved path (no separate config needed)
...
- [ ] Flash UDP firmware, verify readings arrive at master
...
- [ ] ~install.sh~: create ~tmon~ user, create ~/etc/tmon/~ and ~/var/lib/tmon/~
- [ ] ~install.sh~: pip install master package, copy config files
- [ ] ~install.sh~: install firmware binaries to ~/var/lib/tmon/firmware/~
- [ ] systemd service files: ~tmond-serial.service~, ~tmond-udp.service~
- [ ] systemd service file: ~tmon-panel.service~ (gunicorn serving Flask app)
- [ ] ~install.sh~: enable and start selected services
- [ ] ~uninstall.sh~ and ~make uninstall~ target
- [ ] ~make install~ target that invokes ~install.sh~

*** Firmware flash tool

Convenience script for flashing ESP32s from the Pi.

- [ ] ~tm-flash~ script: ~--mode={serial,udp} --addr=N~
- [ ] Reads WiFi/host settings from ~/etc/tmon/~ config (not CLI args)
- [ ] Invokes esptool with correct firmware from ~/var/lib/tmon/firmware/~
...
- [ ] Check LED behavior sanity (stub vs real hardware alignment)
