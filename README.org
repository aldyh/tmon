...
- *Server (Raspberry Pi):* Python daemon that collects readings from ESP32 sensors and stores them in SQLite.
- *Transport:* RS-485 (USB adapter, server polls) or UDP over WiFi (sensors push).
- *Sensors (ESP32):* Each reads up to *4 NTC thermistors*.
- *Protocol:* Simple custom binary frames with *CRC-16/MODBUS*.
- *Scale:* 1-4 sensors; designed to stay small and stable.

** Directory layout

- ~server/~ -- Python 3.11+ polling daemon
...
Firmware must be built on the *dev machine* (x86, with PlatformIO) and
copied to the Pi before installing:
...
The installer also copies pre-built firmware to ~/var/lib/tmon/firmware/~
for use by ~tmon-flash~.

** Configuration

Config files live in ~/etc/tmon/~:

- ~config-485.toml~ -- RS-485 transport: serial port, baud rate, sensor list, polling interval.
- ~config-udp.toml~ -- UDP transport: listen port, push interval.
- ~wifi.toml~ -- WiFi credentials for UDP sensors (copy from ~wifi.toml.example~ and fill in).
...
Use the ~tmon-flash~ tool to flash pre-built firmware onto ESP32 sensors:

#+begin_example
tmon-flash --mode=serial --addr=1          # RS-485 sensor at address 1
tmon-flash --mode=udp --addr=5             # UDP sensor at address 5
#+end_example
...
Individual test targets: ~check-server~, ~check-sensor~, ~check-integration~, ~check-demo~.
...
An alternative to ~tmon-flash~ for development -- flashes one sensor at a time
without needing pre-built firmware:

#+begin_example
make flash-sensor-485 SENSOR_ADDR=1    # RS-485 transport
make flash-sensor-udp SENSOR_ADDR=1    # UDP transport
#+end_example
