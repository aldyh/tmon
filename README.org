...
- *Master (Raspberry Pi):* Python daemon that polls ESP32 slaves and stores readings in SQLite.
- *Transport:* RS-485 (USB adapter) or WiFi (TCP).
...
WiFi credentials are read from ~master/config-wifi.toml~.

*** Run the daemon

#+begin_example
make run-master       # RS-485 transport (uses master/config.toml)
make run-master-wifi  # WiFi transport (uses master/config-wifi.toml)
#+end_example
