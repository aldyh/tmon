...
- ~tmon.toml~ -- Main configuration: database path, ~[rs485]~ section (serial port, baud rate, client list, polling interval), ~[udp]~ section (listen port, push interval).
- ~wifi.toml~ -- WiFi credentials for UDP clients (copy from ~wifi.toml.example~ and fill in).

The ~--transport~ flag on the daemon CLI selects which transport to use (rs485 or udp).  The transport is not stored in the config file.
