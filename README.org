...
*left* one (labeled USB on the PCB) -- it appears as ~/dev/ttyACM*~.
The right UART port (labeled COM) appears as ~/dev/ttyUSB*~ and may
conflict with the USB→485 adapter.  See the [[file:docs/wiring.org][Wiring Reference]] for a
diagram.
