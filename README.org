...
[wifi]
port = 5555               # listen port for WiFi clients
push_interval = 5         # how often clients push readings (seconds)
#+end_example

For WiFi clients, you also need ~/etc/tmon/wifi.toml~ with your network credentials:
...
Enable the two daemons (one for WiFi and one for serial):

#+begin_example
sudo systemctl enable --now tmond-serial   # RS-485 server
sudo systemctl enable --now tmond-wifi     # WiFi push server
#+end_example
