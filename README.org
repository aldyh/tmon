...
- *Transport:* RS-485 (USB adapter, master polls) or UDP over WiFi (slaves push).
...
- ~config-485.toml~ -- RS-485 transport: serial port, baud rate, slave list, polling interval.
...
#+begin_example
make run-master-485   # RS-485 (uses master/config-485.toml)
...
#+begin_example
make flash-slave-485 SLAVE_ADDR=1    # RS-485 transport
