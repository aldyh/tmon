...
* TODO one toml to rule them all
...
** Plan
*** Unified file layout
One file: =server/tmon.toml=.  Top-level keys for the database and client
list; transport-specific settings in their own sections:
...
*** Transport selection
Both daemons (RS-485 and UDP) run simultaneously from the same file.
Each daemon selects its section via a =--transport= CLI flag
(e.g. =tmon-server --transport rs485=).  The flag is NOT stored in the
TOML -- it belongs in the systemd unit or the Makefile invocation.

*** wifi.toml stays separate
=wifi.toml= holds Wi-Fi SSID/password (secrets).  It is gitignored and
only read by =tmon-flash=.  No change here.

*** No backward compatibility
Single-user home project.  Old =config-485.toml= and =config-udp.toml=
are deleted outright; no migration or fallback logic.

*** Files to change
- =server/config.py= -- load unified file, accept transport parameter
- =server/daemon.py= -- pass =--transport= to config loader
- =server/test_config.py= -- update fixtures to unified format
- =tmon-flash= -- point at =server/tmon.toml= for client list
- =install.sh= -- install one config file, update systemd units
- systemd units -- add =--transport rs485= / =--transport udp=
- =Makefile= -- single config path for both daemons
- =docs/= -- update any config references
* TODO flesh out Configuration section in README
