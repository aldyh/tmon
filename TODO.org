...
- ~make run-panel~ generates data and starts the server in one step.
- ~make check-panel~ runs the panel test suite.
...
Slave side:
- ~dispatch.cpp~: Frame dispatch (responds to POLL with REPLY)
...
- ~make run-master-485~ for RS-485, ~make run-master-wifi~ for WiFi
...
|                  | dispatch.cpp: 69              | (no dispatch needed)          |
...
* DONE Binary-patch per-device configuration into firmware

Replaced compile-time build flags (~slave_addr.py~, ~wifi_config.py~)
with binary-patchable marker arrays in firmware.  ~deploy/tmon-patch~
finds ~@@MARKER_XXX@@~ byte patterns and overwrites them with real
configuration values before flashing.

** Notes

New files:
- ~slave/include/config.h~, ~slave/src/config.cpp~: marker arrays and
  ~config_init()~ that parses patched bytes at boot.
- ~deploy/tmon-patch~: Python binary patcher (addr, ssid, pass, host,
  port).

Changes:
- ~main_485.cpp~, ~main_udp.cpp~: use ~config_slave_addr~ and WiFi
  globals from config.h instead of compile-time build flags.
- ~platformio.ini~: removed all ~extra_scripts~ lines.
- ~tmon-flash~: patches generic binary before flashing.  For UDP mode,
  reads ~wifi.toml~ and ~config-udp.toml~ for WiFi/UDP settings.
- ~Makefile~: ~firmware~ target builds 2 generic binaries (not 8).
  ~flash-slave*~ targets build then call ~tmon-flash~.
- ~install.sh~: installs ~tmon-patch~ alongside ~tmon-flash~.

Deleted:
- ~slave/slave_addr.py~, ~slave/wifi_config.py~.

All 217 tests pass (114 master + 40 slave + 8 integration + 55 panel).
...
** Notes
Simplified LED API to two states: OFF and ERROR (blinking red, 500ms
on/off).  Removed watchdog timer, green TX blink, and led_blink().
Added led_clear().  Serial firmware no longer uses LED at all.  UDP
firmware stays connected to WiFi permanently (no deep sleep), blinks
red only when WiFi is disconnected.  LED tests reduced from 14 to 7.
* DONE Boot button triggers address query
...
** Notes
Added LED_IDENTIFY state (blinks yellow N times at 300ms on/off, then
restores prior state).  Both firmware versions poll GPIO 0 (boot button)
with 500ms debounce.  main_udp.cpp blocking delay replaced with 10ms
polling loop.  7 new LED tests (14 total).
* DONE rename build-slave to build-slave-485
and flash-slave to flash-slave-485
** Notes
Renamed in Makefile and README.org.
* DONE should config.toml be config-485.toml
For consistency, since we have config-udp.toml, should config.toml be config-485.toml?

485 and udp implementations should both be first class citizens.

Or do we have config.toml, because that's a default of sorts and it must be named that?

** Notes
Renamed to config-485.toml. The filename is never used as a hardcoded
default -- it's always passed explicitly as a CLI positional arg, so
neither transport looks like the "default".
* DONE Document composite index in storage.org
~storage.py~ creates a composite index:

#+begin_example
CREATE INDEX IF NOT EXISTS idx_readings_addr_ts ON readings (addr, ts);
#+end_example

This was added in commit 655c689 and noted in PLAN.org, but
~docs/storage.org~ has no mention of it.  The Schema section should
list the index alongside the table definition.

** Notes
Added an "Indexes" subsection after "Column details" in docs/storage.org
documenting the idx_readings_addr_ts composite index on (addr, ts).
* DONE Fix uninstall.sh: tmon-patch not removed
~install.sh~ installs both ~tmon-flash~ and ~tmon-patch~ to
~/usr/local/bin/~, but ~uninstall.sh~ only removes ~tmon-flash~ (line
49).  ~tmon-patch~ is left behind after uninstall.

One-line fix: add ~rm -f /usr/local/bin/tmon-patch~ after the existing
~rm -f /usr/local/bin/tmon-flash~ line.
** Notes
Added ~rm -f /usr/local/bin/tmon-patch~ to uninstall.sh.
* DONE Validate sensor address range (1-247) in config.py
~_require_sensors()~ checks that values are ints and the list is
non-empty, but does not validate the 1-247 address range.  A typo like
~sensors = [0]~ or ~sensors = [300]~ passes config validation and only
fails at protocol encode time deep in the poll loop.

Add a range check in ~_require_sensors()~:
...
#+end_example
** Notes
Added range check in ~_require_sensors()~ after the type check.
Two new tests: ~test_sensors_addr_zero~ and ~test_sensors_addr_too_high~.
* DONE Remove misleading push_interval from config-udp.toml
~config-udp.toml~ line 17 contains ~push_interval = 5~ which the server
config loader silently ignores.  This value only matters on the sensor
side (patched into firmware by ~tmon-patch~).  Having it in a server
config file misleads users into thinking they can change the push rate
from the server.

Either remove it from the file, or add a comment making clear it is
informational only and has no effect on the server.
* DONE Add database retention / cleanup mechanism
The daemon purges readings older than 365 days on startup, then
vacuums to reclaim disk space.  See ~Storage.purge()~ in
~server/src/tmon/storage.py~.
* DONE sensor / client rename
Similar to what we did in the README.org
Make sure the makefile targets and the docs are updated as well.
** Notes
Renamed throughout in branch claude/074-sensor-client-rename:
- sensor/ directory -> client/
- Makefile targets: build-sensor-* -> build-client-*, check-sensor -> check-client
- C++ classes: SensorApp -> ClientApp, RS485Sensor -> RS485Client, UDPSensor -> UDPClient
- sensors.h/cpp -> ntc.h/cpp (NTC thermistor code)
- Python: config key sensors -> clients, all docstrings/comments
- TOML: sensors = [...] -> clients = [...]
- Panel code and tests
- All documentation (protocol.org, storage.org, wiring.org, PLAN.org)
* DONE one toml to rule them all
I hate that we have config-485.toml and config-udp.toml.  These should
be merged so we have a config.toml (that has udp and 485 settigs).  it's confusing having two.  though I don't know how the make system will be able to distinguish between the 2.

however, we should have a wifi.toml.example separately.

** Notes
Merged config-485.toml and config-udp.toml into a single tmon.toml with
[rs485] and [udp] sections.  Transport is selected via --transport CLI
flag (not stored in the TOML).  Updated config loader, daemon CLI, all
tests, deploy scripts (tmon-flash, install.sh, systemd units), Makefile,
and docs (README.org, storage.org, PLAN.org).
* DONE flesh out Configuration section in README
The config file descriptions list the files but don't guide the user
on what to actually change (e.g. adding client addresses, picking the
right serial port).  Add brief instructions or examples.

** Notes
Added annotated tmon.toml example (all fields with inline comments),
wifi.toml setup instructions (cp from example, fill in SSID/password/
server IP), and clarifying notes on which fields matter where (e.g.
push_interval only affects firmware at flash time).  Also fixed stale
"sensor" -> "client" in wifi.toml.example.
* DONE Speed up install.sh pip installs on Raspberry Pi
The venv is already reused across reinstalls, but the three ~pip install~
calls are still slow on the Pi because ~server/~ and ~panel/~ are local
directories — pip rebuilds the sdist every time even if nothing changed.
~esptool~ is fast (already satisfied) but the local packages are not.

Simplest fix: add a ~--quick~ flag that skips the pip installs entirely.
Use it when redeploying config, services, or firmware without Python
code changes.  Default path stays unchanged.

Fancier alternatives considered and rejected (not worth the complexity):
- Stamp files with checksums of pyproject.toml or full source trees.
- ~--no-build-isolation~ or ~--no-deps~ tricks (marginal gains).

This is what most projects do: just eat the cost, or provide a fast path
flag for the common "nothing changed in Python" case.

** Notes
~--quick~ skips the entire Python section (venv creation + pip installs).
Normal mode always creates a fresh venv — the old "reuse if exists"
behavior was removed so a full install is a guaranteed clean slate.
~--quick~ errors out if no venv exists from a prior full install.
* CANCELLED Restart running services on reinstall
~install.sh~ uses ~systemctl enable --now~ which only starts a stopped
service.  If daemons are already running, they keep the old code/config
until manually restarted.  The installer should restart (or stop-then-start)
any already-running services so the new config and panel files take effect
immediately.

** Notes
Not worth the complexity for a single-operator home project.  The
installer would need to detect which services are actually running
(RS-485, WiFi, or both) and selectively restart them.  Since the
operator already knows what changed, a manual ~systemctl restart~ is
