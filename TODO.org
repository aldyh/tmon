...
* TODO Remove misleading push_interval from config-udp.toml
...
* TODO Add database retention / cleanup mechanism
The SQLite database grows without bound.  At 4 sensors polling every
30 seconds, that's ~4.2M rows/year (~200MB).  After a few years this
becomes unwieldy.

Options:
1. A CLI command or daemon flag that deletes rows older than N days.
2. A periodic task (cron or built into the daemon loop) that prunes
   old data.
3. Document expected growth and manual cleanup in docs/storage.org.
