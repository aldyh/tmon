...
* CANCELLED Speed up install.sh pip installs on Raspberry Pi
...
** Notes
A ~--quick~ flag was prototyped and reverted.  The only reason to re-run
~install.sh~ is a full reinstall where you want pip.  For config tweaks,
just edit the TOML files and restart the relevant daemon — no installer
needed.  Also removed the venv-reuse ~-d~ check: unconditionally
creating the venv is simpler and the cost is negligible (pip is the slow
part, not venv creation).
