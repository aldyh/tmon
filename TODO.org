...
* TODO Rename Udp to UDP
There are still instances of:
class TestUdpIntegration:
notice Udp awkwardness.  Fix any other places in the code base.
* TODO Implement UDP simulator if needed
We have a simulator for the RS485 slaves, but we have two modes of operation, RS485 and UDP.
We should factor out the 485 bits, such that we have a file with the 485 bits, another one with UDP simulator bits, and a simulator.py for the overall general simulator interface-- probably dispatching to either one depending on how its being run.
