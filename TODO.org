...
- [X] Create ~udp_bus.py~ with simple recv loop
...
Master side:
- ~udp_bus.py~: Simple UDP socket receiver
- ~collector_push.py~: Decodes REPLY frames, stores to DB
- ~daemon.py~: New ~run_push()~ loop for UDP mode
- Renamed ~poller.py~ to ~collector_poll.py~ for consistency
...
Commits:
1. Rename Poller to PollCollector
2. Add UdpBus
3. Add PushCollector
...
| Component        | TCP                      | UDP                       |
|------------------+--------------------------+---------------------------|
| Python bus       | wifi_bus.py: 166         | udp_bus.py: 79            |
| Python collector | (reuses PollCollector)   | collector_push.py: 134    |
...
PollCollector). However, LOC alone is misleading.
...
UDP is easier:
- ~udp_bus.py~ is 79 lines of stateless socket code
...
UDP:
- ~udp_bus.py~: Bind socket, receive datagrams. Trivial.
- ~collector_push.py~: Clean separation. Frame decode, store, done.
...
| Slave total      | 408                           | 381                           |
|------------------+-------------------------------+-------------------------------|
| Master daemon    | bus.py: 69                    | udp_bus.py: 79                |
|                  | collector_poll.py: 159        | collector_push.py: 134        |
