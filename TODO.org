...
* TODO Convert WiFi transport from TCP to UDP push model
...
- [ ] Create ~udp_bus.py~ with simple recv loop
- [ ] Store readings on receipt (no polling logic)
- [ ] Detect offline slaves by timestamp threshold
- [ ] Add ~transport = "udp"~ config option
- [ ] Unit tests with mock UDP

*** Phase 2: Slave UDP push

- [ ] Create ~main_udp.cpp~ with wake/send/sleep cycle
- [ ] Configure interval via build flag (~-DPUSH_INTERVAL_S=60~)
- [ ] Deep sleep between pushes
- [ ] Add ~make flash-slave-udp~ target

*** Phase 3: Cleanup

- [ ] Remove TCP transport (~wifi_bus.py~, ~main_wifi.cpp~)
- [ ] Update docs/wiring.org WiFi section
- [ ] Update docs/protocol.org with push model

** Notes

Discussion notes preserved here for future reference.
