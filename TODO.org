...
* TODO Binary-patch per-device configuration into firmware
...
The NVS approach (Preferences API + ~nvs_partition_gen.py~) would
require ESP-IDF tooling on the Pi, a separate NVS partition image
flashed to a specific offset, and fallback logic in firmware for
missing keys.  That is the right tool for a product with OTA and
factory resets, but overkill here.

A simpler approach: embed fixed-length marker strings in the firmware
source at compile time, then patch them with real values in the binary
post-build.  ESP32 firmware binaries are not checksummed, so this
works cleanly -- just a few lines of perl/sed on the Pi, one binary,
one flash step, same as today.

** Approach

- Define marker strings as fixed-length char arrays in the firmware:
  ~char cfg_ssid[33] = "@@MARKER_SSID@@";~
  ~char cfg_pass[65] = "@@MARKER_PASS@@";~
  ~char cfg_addr[4]  = "@@A";~
  The fixed length ensures the compiler allocates enough space in the
  binary for the real values to be written over the markers.
- Build one firmware per transport mode (serial, udp) with the markers
  in place.
- In ~tmon-flash~, patch the binary before flashing:
  ~perl -pi -e "s/\@\@MARKER_SSID\@\@.{17}/sprintf('%-33s',$ssid)/e" fw.bin~
  Or equivalent sed/python one-liner.
- Flash the patched binary as usual with ~esptool.py~.

** Scope

- Firmware: replace build-flag reads with marker char arrays.
- ~tmon-flash~: add a patch step before flashing.
